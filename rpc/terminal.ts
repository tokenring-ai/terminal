...
export default createRPCEndpoint(TerminalRpcSchema, {
  listTerminals(_args, app) {
    ...
    return {
      terminals: Array.from(terminalService.getAllTerminalSessions()).map(
        ([, item]) => ({
          name: item.name,
  ...
  async spawnTerminal(args, app) {
    ...
    const terminalName = await terminalService.createSession({
      providerName:
        args.providerName ?? terminalService.getAvailableProviders()[0],
  ...
  async sendInput(args, app) {
    await app
      .requireService(TerminalService)
      .sendInput(args.terminalName, args.input);
    return {success: true};
  },
  ...
  async getCompleteOutput(args, app) {
    const output = await app
      .requireService(TerminalService)
      .readFullOutput(args.terminalName);
    return {output};
  },
