...
export async function execute(
  {command, dangerouslyDisableSandbox}: z.output<typeof inputSchema>,
  agent: Agent,
  ...
  const confirmed = await agent.askForApproval({
    message: dangerouslyDisableSandbox ?
  ...
  const terminalName = await terminalService.createSession({
    ...
    workingDirectory,
    isolation: dangerouslyDisableSandbox
      ? 'none'
      : activeTerminalProvider.supportedIsolationLevels.includes('sandbox') ? 'sandbox' : 'none',
  });
  ...
  const result = await terminalService.readOutput(
    terminalName,
    {
      fromPosition: 0,
      ...interactiveConfig
...
const inputSchema = z.object({
  ...
  dangerouslyDisableSandbox: z.boolean().default(false).describe("Disables the sandbox, which can be dangerous.")
});
